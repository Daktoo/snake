...
public:
    ...
    void processInput() {
        ...
        while (auto eventOpt = window.pollEvent()) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                if (event.key.code == sf::Keyboard::Key::W && dir != DOWN) dir = UP;
                else if (event.key.code == sf::Keyboard::Key::S && dir != UP) dir = DOWN;
                else if (event.key.code == sf::Keyboard::Key::A && dir != RIGHT) dir = LEFT;
                else if (event.key.code == sf::Keyboard::Key::D && dir != LEFT) dir = RIGHT;
                else if (event.key.code == sf::Keyboard::Key::Q) window.close();
            }
        }
    }
    ...
    void update(float dt) {
        ...
        if (newHead.x < 0 || newHead.x >= WINDOW_WIDTH / CELL_SIZE || newHead.y < 0 || newHead.y >= WINDOW_HEIGHT / CELL_SIZE || std::find(snake.begin(), snake.end(), newHead) != snake.end()) {
            window.close();
            return;
        }
