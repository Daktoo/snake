...
public:
    SnakeGame() : window(sf::VideoMode(sf::Vector2u{WINDOW_WIDTH, WINDOW_HEIGHT}), "Snake"), dir(RIGHT), score(0), moveDelay(0.15f), moveTimer(0.f) {
    ...
    void update(float dt) {
        ...
        if (newHead.x < 0 || newHead.x >= WINDOW_WIDTH / CELL_SIZE || newHead.y < 0 || newHead.y >= WINDOW_HEIGHT / CELL_SIZE) { window.close(); return; }
        for (auto& s : snake) if (s == newHead) { window.close(); return; }

        snake.insert(snake.begin(), newHead);
        if (newHead == food) { score++; placeFood(); }
        else snake.pop_back();
